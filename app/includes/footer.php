    ...
    <div data-simplebar class="h-100">
      <div class="card mb-0 p-3">
        <div class="alert alert-warning" role="alert">
          <strong><?= h(__('theme_customize')) ?></strong> <?= h(__('theme_customize_sub')) ?>
        </div>
        ...
        <div class="small text-muted mt-3">
          <?php
          // Use translation with fallback (translatable fallback)
          $previewNote = __('theme_note_preview');
          if (!$previewNote || $previewNote === 'theme_note_preview') {
            $previewNote = __('theme_note_preview_fallback') ?: 'Perubahan di sini adalah pratayang. Untuk simpan kekal, guna halaman Tetapan Sistem.';
          }
          echo h($previewNote);
          ?>
        </div>
...
(function() {
  ...
  function showThemeToast(message) {
    ...
    if (typeof Swal !== 'undefined') {
      Swal.fire({
        icon: 'success',
        title: '<?= h(__('theme_applied') ?: 'Theme Applied') ?>',
        text: message,
        toast: true,
        position: 'top-end',
   ...
   * @param {string} displayName - Display name for toast notification
   * @returns {void}
   */
  function setupThemeListener(inputName, allowedValues, storageKey, targetElement, attributeName, defaultValue, displayName) {
    ...
    inputs.forEach(input => {
      input.addEventListener('change', function(e) {
        ...
        debounce(`theme-${inputName}`, function() {
          ...
          if (displayName) {
            showThemeToast(`${displayName} changed to ${value}`);
          }
        }, DEBOUNCE_DELAY);
    });
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const showUpdatingMessage = function() {
    if (typeof Swal !== 'undefined') {
      Swal.fire({
        icon: 'info',
        title: <?= json_encode(__('footer_content_updating_title') ?: 'Maklumat', JSON_UNESCAPED_UNICODE) ?>,
        text: <?= json_encode(__('footer_content_updating') ?: 'Kandungan sedang dikemaskini.', JSON_UNESCAPED_UNICODE) ?>,
        confirmButtonText: <?= json_encode(__('footer_content_updating_ok') ?: 'OK', JSON_UNESCAPED_UNICODE) ?>,
        background: (document.documentElement.getAttribute('data-bs-theme') === 'dark') ? '#1e1e2d' : '#fff',
      });
    } else {
      // Fallback jika SweetAlert2 belum loaded
      alert(<?= json_encode(__('footer_content_updating') ?: 'Kandungan sedang dikemaskini.', JSON_UNESCAPED_UNICODE) ?>);
    }
  };
