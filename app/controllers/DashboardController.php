...
class DashboardController
{
    ...
    public function __construct()
    {
        ...
        try {
            ...
            $prof        = $userModel->getProfile();        // fallback guna $_SESSION['f_stafID']
    ...
    {
        if ($this->db === null) {
            // ✅ Sybase aktif (ikut SYBASE_ACTIVE_BASE)
            $this->db = Database::pdoSybaseActive();
        }
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'DBPROCESS') !== false || 
                ...
                Database::clearInstance('sybase_active');
                $this->db = null;
                $this->db = Database::pdoSybaseActive();
            } else {
