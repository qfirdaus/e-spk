...
class PemetaanKursusUniversitiPLO {
    private PDO $dbSPK;
    ...
    public function getPLOList(string $sesi): array {
        if (empty($sesi)) return [];

        $sql = "SELECT id_plo, kod_plo FROM spk_tplo WHERE status_aktif = 1 AND kod_sesi = :sesi AND program_universiti = 'Universiti' ORDER BY kod_plo";
        $stmt = $this->dbSPK->prepare($sql);
        $stmt->execute([':sesi' => $sesi]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getPemetaanData(string $sesi): array {
        if (empty($sesi)) return [];

        $sqlKursus = "SELECT id_kursus, kod_kursus, term_pengajian 
                      FROM spk_tkursus 
                      WHERE term_pengajian = :sesi AND program_universiti = 'Universiti' AND penyelaras_kursus IS NOT NULL 
                      ORDER BY kod_kursus";
        $stmtK = $this->dbSPK->prepare($sqlKursus);
        $stmtK->execute([':sesi' => $sesi]);
        $courses = $stmtK->fetchAll(PDO::FETCH_ASSOC);

        $sqlMap = "SELECT id_kursus, id_plo FROM v_spk_penetapan_kursus_plo WHERE term_pengajian = :sesi";
        $stmtM = $this->dbSPK->prepare($sqlMap);
        $stmtM->execute([':sesi' => $sesi]);
