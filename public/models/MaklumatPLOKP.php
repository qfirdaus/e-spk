...
class MaklumatPLOKP
{
    ...
    public function getPloList(string $sesiPlo, string $programPlo): array
    {
        $sql = "SELECT 
                    p.*,
                    GROUP_CONCAT(st.id_peo ORDER BY st.id_peo SEPARATOR ', ') AS senarai_id_peo,
                    GROUP_CONCAT(st.kod_peo ORDER BY st.kod_peo SEPARATOR ', ') AS senarai_kod_peo,
                    GROUP_CONCAT(st.keterangan_bm ORDER BY st.kod_peo SEPARATOR ' | ') AS senarai_keterangan_peo
                FROM spk_tplo p
                LEFT JOIN spk_tpenetapan_peo_plo stpp ON p.id_plo = stpp.id_plo
                LEFT JOIN spk_tpeo st ON stpp.id_peo = st.id_peo AND st.status_aktif = 1
                WHERE p.status_aktif = 1 
                AND p.kod_sesi = :sesi 
                AND p.kod_program = :program
                GROUP BY p.id_plo";
    ...
    public function getPeoListByProgram(string $sesi, string $kodProgram): array
    {
        $sesiPrefix = substr($_SESSION["sesiplo"] ?? '', 0, -1) . '%';

        $sql = "SELECT id_peo, kod_peo, keterangan_bm 
                FROM spk_tpeo 
                WHERE status_aktif = 1 
                AND kod_sesi LIKE :sesi_prefix
                AND kod_program = :kod_program
                ORDER BY kod_peo ASC";

        $stmt = $this->pdoSPK->prepare($sql);
        $stmt->execute([
            ':sesi_prefix' => $sesiPrefix,
