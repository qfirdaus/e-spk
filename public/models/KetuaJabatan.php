...
class KetuaJabatan
{
    ...
    public function deleteDataHeadDept(array $data): bool 
    {
        ...
        try {
            ...
            $checkSql = "SELECT f_userID, f_groupID FROM tbl_m_user WHERE f_stafID = :stafID LIMIT 1";
            $checkStmt = $this->pdoSPK->prepare($checkSql);
            $checkStmt->execute([':stafID' => $stafID]);
            $userMain = $checkStmt->fetch(\PDO::FETCH_ASSOC);
