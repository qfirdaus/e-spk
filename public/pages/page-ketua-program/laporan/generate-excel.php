...
$data = $controller->getExcelReportData((string)$kursusId);

if (!$data) {
    die("Rekod kursus tidak dijumpai.");
}
...
$clos       = $data['clos'] ?? [];
$skills     = $data['skills'] ?? [];
$cco_list   = $data['slt_cco'] ?? [];
$cont_list  = $data['slt_ca'] ?? [];
$final_list = $data['slt_fa'] ?? [];
$refs       = $data['references'] ?? [];

$clo_count        = count($clos);
$clo_num_row      = $clo_count + 1;
$skill_count      = count($skills);
$cco_count        = count($cco_list);
$continuous_count = count($cont_list);
$final_count      = count($final_list);

$prereq = $data['prereq'] ?? [];
$prerequisite_str = trim(implode(" ", [
    $prereq['f240psyarat1'] ?? '', $prereq['f240psyarat2'] ?? '',
    $prereq['f240psyarat3'] ?? '', $prereq['f240psyarat4'] ?? '',
    $prereq['f240psyarat5'] ?? ''
]));
        ...
        <table class='tb4' border='1'> 
            <tbody>
                ...
                <tr>
                    ...
                    <td colspan='<?= $plo_num_col ?>'><?= htmlspecialchars($course['gelar_nama'] ?? '') ?> - <?= htmlspecialchars($course['penyelaras_kursus'] ?? '') ?></td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan='<?= $plo_num_col ?>'><?= htmlspecialchars($prerequisite_str) ?></td>
                </tr>
                ...
                <tr>
                    ...
                    <?php for ($col = 1; $col <= $plo_num_max; $col++): ?>
                        ...
                        <?php endif; ?>
                    <?php endfor; ?>
                    <td><?= htmlspecialchars(implode(', ', $clo['methods'] ?? [])) ?></td>
                    <td><?= htmlspecialchars(implode(', ', $clo['assessments'] ?? [])) ?></td>
                </tr>
                <?php endforeach; ?>
                ...
                <?php else: ?>
                    <?php $col = 1; foreach ($skills as $skill): ?>
                        <tr>
                            <?php if ($col == 1): ?>
                ...
                <?php $totalslt_cco = 0; ?>
                <?php foreach ($cco_list as $cco): ?>
                    <tr style='text-align: center'>
                        <td colspan='7' style='text-align: left'><?= htmlspecialchars($cco["content_outline"] ?? '') ?></td>
                        <td><?= htmlspecialchars($cco["kod_clo"] ?? '') ?></td>
                        <td><?= htmlspecialchars((string)($cco["f2f_lecture"] ?? '0')) ?></td>
                        <td><?= htmlspecialchars((string)($cco["f2f_tutorial"] ?? '0')) ?></td>
                        <td><?= htmlspecialchars((string)($cco["f2f_practical"] ?? '0')) ?></td>
                        <td><?= htmlspecialchars((string)($cco["f2f_others"] ?? '0')) ?></td>
                        <td><?= htmlspecialchars((string)($cco["nf2f_guided"] ?? '0')) ?></td>
                        <td><?= htmlspecialchars((string)($cco["nf2f_independent"] ?? '0')) ?></td>
                        <th style='text-align: center'><?= htmlspecialchars((string)($cco["slt"] ?? '0')) ?></th>
                    </tr>
                ...
                $col = 1; 
                ?>
                <?php foreach ($cont_list as $cont): ?>
                    <tr>
                        <th style='text-align: center'><?= $col ?></th>
                        <td colspan='6'><?= htmlspecialchars($cont["penilaian"] ?? '') ?></td>
                        <td colspan='3' style='text-align: center'><?= htmlspecialchars((string)($cont["percentage"] ?? '0')) ?></td>
                        <td colspan='2' style='text-align: center'><?= htmlspecialchars((string)($cont["f2f"] ?? '0')) ?></td>
                        <td colspan='2' style='text-align: center'><?= htmlspecialchars((string)($cont["nf2f"] ?? '0')) ?></td>
                        <th style='text-align: center'><?= htmlspecialchars((string)($cont["slt"] ?? '0')) ?></th>
                    </tr>
                    <?php 
                ...
                $col = 1; 
                ?>
                <?php foreach ($final_list as $fin): ?>
                    <tr>
                        <th style='text-align: center'><?= $col ?></th>
                        <td colspan='6'><?= htmlspecialchars($fin["penilaian"] ?? '') ?></td>
                        <td colspan='3' style='text-align: center'><?= htmlspecialchars((string)($fin["percentage"] ?? '0')) ?></td>
                        <td colspan='2' style='text-align: center'><?= htmlspecialchars((string)($fin["f2f"] ?? '0')) ?></td>
                        <td colspan='2' style='text-align: center'><?= htmlspecialchars((string)($fin["nf2f"] ?? '0')) ?></td>
                        <th style='text-align: center'><?= htmlspecialchars((string)($fin["slt"] ?? '0')) ?></th>
                    </tr>
                    <?php 
