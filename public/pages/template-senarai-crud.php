...
require_once __DIR__ . '/../controllers/TemplateSenaraiCrudController.php';

$controllerClass = 'TemplateSenaraiCrudController';
...
$PAGE_TITLE = t('template_senarai_crud_page_title', 'Template Senarai CRUD');
...
<head>
  ...
  <link href="<?= base_url('assets/css/pages/template-senarai-crud.css') ?>?v=<?= h($version) ?>" rel="stylesheet">
  ...
  <div class="content-page">
    <div class="content">
      <div class="container-fluid">
        ...
        <div class="row mb-3">
          <div class="col-12">
            <div class="page-title-box d-flex justify-content-between align-items-center flex-wrap">
              <h4 class="page-title"><i class="ri-survey-line me-1"></i> <?= h($PAGE_TITLE) ?></h4>
        ...
        <div class="row">
          <div class="col-12">
            <div class="card crud-datatable-card">
              <div class="card-body">
                <div class="crud-datatable-intro">
                  <div>
                    <h5 class="card-title mb-1"><?= h(t('template_senarai_crud_table_title', 'Management Listing')) ?></h5>
                    <p class="text-muted mb-0"><?= h(t('template_senarai_crud_table_subtitle', 'Use this generated page as the starting point for admin modules that need listing, modal CRUD, and polished interaction flow.')) ?></p>
                  </div>
                  <button type="button" id="btnSampleAction" class="btn btn-primary sync-groups-btn">
                    <i class="ri-add-line"></i><span><?= h(t('template_senarai_crud_btn_sample', 'Add New')) ?></span>
                  </button>
                </div>
                <div class="table-responsive dt-standard">
                  <table class="table table-bordered align-middle w-100" id="userDT">
                    <thead>
                      <tr>
                        <th class="col-bil"><?= h(t('template_senarai_crud_col_no', 'No.')) ?></th>
                        <th class="col-nama"><?= h(t('template_senarai_crud_col_name', 'Name')) ?></th>
                        <th class="col-jabatan"><?= h(t('template_senarai_crud_col_department', 'Department')) ?></th>
                        <th class="col-group"><?= h(t('template_senarai_crud_col_group', 'Group')) ?></th>
                        <th class="col-akses"><?= h(t('template_senarai_crud_col_access', 'Access')) ?></th>
                        <th class="col-actions"><?= h(t('template_senarai_crud_col_actions', 'Actions')) ?></th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php foreach ($rows as $index => $row): ?>
                        <?php
                          ...
                          $accessLabel = $access === 1
                            ? t('template_senarai_crud_access_allowed', 'Allowed')
                            : t('template_senarai_crud_access_blocked', 'Blocked');
                          $accessClass = $access === 1 ? 'is-allowed' : 'is-blocked';
                        ?>
                        <tr data-row-index="<?= h((string)$index) ?>">
                          <td class="col-bil"></td>
                          <td class="col-nama">
                            <span class="truncate-1line" data-bs-toggle="tooltip" data-bs-custom-class="template-tooltip" title="<?= h(trim($name . ($identifier !== '' ? ' (' . $identifier . ')' : ''))) ?>">
                              <?= h($name) ?><?php if ($identifier !== ''): ?> (<?= h($identifier) ?>)<?php endif; ?>
                            </span>
                          </td>
                          <td class="col-jabatan"><span class="truncate-1line" data-bs-toggle="tooltip" data-bs-custom-class="template-tooltip" title="<?= h($department) ?>"><?= h($department) ?></span></td>
                          <td class="col-group">
                            <span class="cell-inline">
                              <span class="group-chip truncate-1line" data-bs-toggle="tooltip" data-bs-custom-class="template-tooltip" title="<?= h($groupName) ?>"><?= h($groupName) ?></span>
                            </span>
                          </td>
                          <td class="col-akses">
                            <span class="access-chip <?= h($accessClass) ?> truncate-1line" data-bs-toggle="tooltip" data-bs-custom-class="template-tooltip" title="<?= h($accessLabel) ?>"><?= h($accessLabel) ?></span>
                          </td>
                          <td class="col-actions">
                            <button type="button" class="btn btn-outline-warning btn-sm js-view-row" data-row='<?= h((string)$rowJson) ?>' title="<?= h(t('template_senarai_crud_btn_view', 'View')) ?>">
                              <i class="ri-eye-line"></i>
                            </button>
                            <button type="button" class="btn btn-outline-primary btn-sm js-edit-row" data-row='<?= h((string)$rowJson) ?>' title="<?= h(t('template_senarai_crud_btn_edit', 'Edit')) ?>">
                              <i class="ri-pencil-line"></i>
                            </button>
                            <button type="button" class="btn btn-outline-danger btn-sm js-delete-row" data-row='<?= h((string)$rowJson) ?>' title="<?= h(t('template_senarai_crud_btn_delete', 'Delete')) ?>">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="sampleEditModalTitle">
          <i class="ri-pencil-line"></i> <?= h(t('template_senarai_crud_modal_edit_title', 'Edit Record')) ?>
        </h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="<?= h(t('template_senarai_crud_btn_close', 'Close')) ?>"></button>
      </div>
      <div class="modal-body">
        <form class="sample-form-shell" id="sampleEditForm">
          ...
          <div>
            <label for="sampleEditName" class="form-label"><?= h(t('template_senarai_crud_field_name', 'Name')) ?></label>
            <input type="text" class="form-control" id="sampleEditName" value="">
          </div>
          <div>
            <label for="sampleEditDepartment" class="form-label"><?= h(t('template_senarai_crud_field_department', 'Department')) ?></label>
            <input type="text" class="form-control" id="sampleEditDepartment" value="">
          </div>
          <div>
            <label for="sampleEditGroup" class="form-label"><?= h(t('template_senarai_crud_field_group', 'Group')) ?></label>
            <input type="text" class="form-control" id="sampleEditGroup" value="">
          </div>
          <div>
            <label for="sampleEditAccess" class="form-label"><?= h(t('template_senarai_crud_field_access', 'Access')) ?></label>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><?= h(t('template_senarai_crud_btn_cancel', 'Cancel')) ?></button>
        <button type="button" class="btn btn-primary" id="sampleEditSaveBtn"><?= h(t('template_senarai_crud_btn_save', 'Save')) ?></button>
      </div>
    </div>
  </div>
</div>

<div class="modal fade sample-modal sample-modal--view" id="sampleViewModal" tabindex="-1" aria-hidden="true" aria-labelledby="sampleViewModalTitle">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="sampleViewModalTitle">
          <i class="ri-eye-line"></i> <?= h(t('template_senarai_crud_modal_view_title', 'View Record')) ?>
        </h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="<?= h(t('template_senarai_crud_btn_close', 'Close')) ?>"></button>
      </div>
      <div class="modal-body">
        <div class="sample-view-grid">
          <div>
            <div class="sample-view-label"><?= h(t('template_senarai_crud_field_name', 'Name')) ?></div>
            <div class="sample-view-value" id="sampleViewName">-</div>
          </div>
          <div>
            <div class="sample-view-label"><?= h(t('template_senarai_crud_field_department', 'Department')) ?></div>
            <div class="sample-view-value" id="sampleViewDepartment">-</div>
          </div>
          <div>
            <div class="sample-view-label"><?= h(t('template_senarai_crud_field_group', 'Group')) ?></div>
            <div class="sample-view-value" id="sampleViewGroup">-</div>
          </div>
          <div>
            <div class="sample-view-label"><?= h(t('template_senarai_crud_field_access', 'Access')) ?></div>
            <div class="sample-view-value" id="sampleViewAccess">-</div>
          </div>
          <div class="sample-view-span">
            <div class="sample-view-label"><?= h(t('template_senarai_crud_field_description', 'Description')) ?></div>
            <div class="sample-view-value" id="sampleViewDescription">-</div>
          </div>
          <div>
            <div class="sample-view-label"><?= h(t('template_senarai_crud_field_updated_at', 'Last Updated')) ?></div>
            <div class="sample-view-value" id="sampleViewUpdatedAt">-</div>
          </div>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><?= h(t('template_senarai_crud_btn_close', 'Close')) ?></button>
      </div>
    </div>
  </div>
</div>
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    function accessLabel(flag) {
      return String(flag) === '1'
        ? <?= json_encode(t('template_senarai_crud_access_allowed', 'Allowed')) ?>
        : <?= json_encode(t('template_senarai_crud_access_blocked', 'Blocked')) ?>;
    }
    ...
    jQuery(document).on('click', '.js-delete-row', function(){
      ...
      Swal.fire({
        icon: 'warning',
        title: <?= json_encode(t('template_senarai_crud_delete_title', 'Delete Sample Record?')) ?>,
        text: <?= json_encode(t('template_senarai_crud_delete_text', 'This is only a frontend sample. No backend action will be triggered.')) ?>,
        showCancelButton: true,
        confirmButtonText: <?= json_encode(t('template_senarai_crud_btn_yes', 'Yes')) ?>,
        cancelButtonText: <?= json_encode(t('template_senarai_crud_btn_no', 'No')) ?>
      }).then(function(result){
        if (!result.isConfirmed) return;
        Swal.fire({
          icon: 'success',
          title: <?= json_encode(t('template_senarai_crud_delete_success_title', 'Sample Delete Complete')) ?>,
          text: (data.name || 'Record') + ' ' + <?= json_encode(t('template_senarai_crud_delete_success_text', 'was processed as a sample action without backend interaction.')) ?>,
          confirmButtonText: <?= json_encode(t('template_senarai_crud_btn_ok', 'OK')) ?>
        });
      });
    });
    ...
    jQuery('#sampleEditSaveBtn').on('click', function(){
      ...
      Swal.fire({
        icon: 'success',
        title: <?= json_encode(t('template_senarai_crud_edit_success_title', 'Sample Save Complete')) ?>,
        text: <?= json_encode(t('template_senarai_crud_edit_success_text', 'The sample edit flow completed without sending any data to the backend.')) ?>,
        confirmButtonText: <?= json_encode(t('template_senarai_crud_btn_ok', 'OK')) ?>
      });
    });
    ...
    jQuery('#sampleAddSaveBtn').on('click', function(){
      ...
      Swal.fire({
        icon: 'success',
        title: <?= json_encode(t('template_senarai_crud_sample_add_success_title', 'Sample Add Complete')) ?>,
        text: <?= json_encode(t('template_senarai_crud_sample_add_success_text', 'The sample add flow completed without sending any data to the backend.')) ?>,
        confirmButtonText: <?= json_encode(t('template_senarai_crud_btn_ok', 'OK')) ?>
      });
    });
