...
require_once __DIR__ . '/../models/KetuaJabatan.php';

class KetuaJabatanController
{
    private KetuaJabatan $model;
    private PDO $pdoSPK;
    ...
    {        
        ...
        $this->model = new KetuaJabatan($this->pdoSPK, $this->pdoStudent, $this->pdoStaff);   
    }
    ...
    public function getHalamanData(): array
    {
        try {
            return [
                'list_ketua_jabatan' => $this->model->getKetuaJabatanList(),              
            ];
        } catch (Throwable $e) {
    ...
    // Save Ketua Jabatan Baharu
    public function saveHeadDept($current_stafID, $formData)
    {
        try {          
            $formData['created_by'] = $current_stafID;

            $isSaved = $this->model->addHeadDeptBaharu($formData);
