jQuery(document).ready(function ($) {
    ...
    if ($('#dataTeachMethodDT').length > 0) {
        if (typeof $.fn.DataTable !== 'undefined') {
            $('#dataTeachMethodDT').DataTable({
                responsive: false, 
                scrollX: false,     
                autoWidth: false,
                ordering: false,   
...
jQuery(function () {
    ...
    if (modalKemaskini) {
        modalKemaskini.addEventListener('show.bs.modal', function (event) {
            ...
            const idTeachMethod = button.getAttribute('data-idTeachMethod');
            const teachMethod = button.getAttribute('data-teachMethod');

            const modal = jQuery(this);

            modal.find('#txtidkaedah_pengajaran_edit').val(idTeachMethod);
            modal.find('#txtkaedah_pengajaran_edit').val(teachMethod);
        });
    }    
});
...
// submit Teaching Method baharu
const btnTambahTeachMethod = document.getElementById('btnTambahTM');
if (btnTambahTeachMethod) {
    btnTambahTeachMethod.addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                submitTeachMethod(form);
            }
        });
    });
}

function submitTeachMethod(formElement) {
    const formData = new FormData(formElement);
    const controllerUrl = base_url + 'pages/page-admin/kaedah-pengajaran/submit-teach-method.php';
...
const btnKemaskiniTeachMethod = document.getElementById('btnKemaskiniTeachMethod');
if (btnKemaskiniTeachMethod) {
    btnKemaskiniTeachMethod.addEventListener('click', function(e) {
        ...
        Swal.fire({
            title: 'Adakah anda pasti?',
            text: "Anda mahu menyimpan maklumat kaedah pengajaran yang telah dikemaskini ini?",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                const hiddenInput = document.createElement('input');
                hiddenInput.type = 'hidden';
                hiddenInput.name = 'btnKemaskiniTeachMethod';
                hiddenInput.value = '1';
                form.appendChild(hiddenInput);

                updateTeachMethod(form);
            }
        });
    });
}

function updateTeachMethod(formElement) {
    const formData = new FormData(formElement);
    const controllerUrl = base_url + 'pages/page-admin/kaedah-pengajaran/update-teach-method.php';
...
// delete LOC
function deleteFunc(idTeachMethod) {
    if (!idTeachMethod) return;
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const controllerUrl = base_url + 'pages/page-admin/kaedah-pengajaran/delete-teach-method.php';
            
            const formData = new FormData();
            formData.append('btnHapus', '1');
            formData.append('idTeachMethod', idTeachMethod);
