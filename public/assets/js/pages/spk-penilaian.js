jQuery(document).ready(function ($) {
    ...
    if ($('#dataAssessmentDT').length > 0) {
        if (typeof $.fn.DataTable !== 'undefined') {
            $('#dataAssessmentDT').DataTable({
                responsive: false, 
                scrollX: false,     
                autoWidth: false,
                ordering: false,   
...
jQuery(function () {
    ...
    if (modalKemaskini) {
        modalKemaskini.addEventListener('show.bs.modal', function (event) {
            ...
            const idAssessment = button.getAttribute('data-idAssessment');
            const assessment = button.getAttribute('data-assessment');

            const modal = jQuery(this);

            modal.find('#txtidpenilaian_edit').val(idAssessment);
            modal.find('#txtpenilaian_edit').val(assessment);
        });
    }    
});
...
// submit Assessment baharu
const btnTambahAssessment = document.getElementById('btnTambahAssessment');
if (btnTambahAssessment) {
    btnTambahAssessment.addEventListener('click', function(e) {
        ...
        Swal.fire({
            ...
            text: "Anda mahu menyimpan maklumat penilaian baharu ini?",
            icon: 'question',
...
function submitAssessment(formElement) {
    const formData = new FormData(formElement);
    const controllerUrl = base_url + 'pages/page-admin/penilaian/submit-assessment.php';
...
// update Assessment
const btnKemaskiniAssessment = document.getElementById('btnKemaskiniAssessment');
if (btnKemaskiniAssessment) {
    btnKemaskiniAssessment.addEventListener('click', function(e) {
        ...
        Swal.fire({
            title: 'Adakah anda pasti?',
            text: "Anda mahu menyimpan maklumat penilaian yang telah dikemaskini ini?",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                const hiddenInput = document.createElement('input');
                hiddenInput.type = 'hidden';
                hiddenInput.name = 'btnKemaskiniAssessment';
                hiddenInput.value = '1';
                form.appendChild(hiddenInput);
...
function updateAssessment(formElement) {
    const formData = new FormData(formElement);
    const controllerUrl = base_url + 'pages/page-admin/penilaian/update-assessment.php';
...
// delete LOC
function deleteFunc(idPenilaian) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const controllerUrl = base_url + 'pages/page-admin/penilaian/delete-assessment.php';
            
            const formData = new FormData();
            formData.append('btnHapus', '1');
            formData.append('idPenilaian', idPenilaian);
